...
insert into fixture(name,id) select 'portal-student',student_id from opening_receivables where id=(select id from fixture where name='opening');
update students set user_id='ee000000-0000-0000-0000-000000000003' where id=(select id from fixture where name='portal-student');
insert into user_roles(user_id,role_id) select 'ee000000-0000-0000-0000-000000000003',id from roles where code='STUDENT';
set local role authenticated;
select set_config('request.jwt.claim.sub','ee000000-0000-0000-0000-000000000003',true);
select is((select outstanding_balance from student_opening_balances((select id from fixture where name='portal-student')) where receivable_id=(select id from fixture where name='opening')),4000000::numeric,'Student portal opening balance matches canonical engine');
select is((select remaining_credit from student_customer_credits((select id from fixture where name='portal-student')) where credit_id=(select id from fixture where name='credit')),0::numeric,'Student portal respects voided credit balance');
select is((select count(*) from student_customer_credits('00000000-0000-4000-8000-000000000001')),0::bigint,'Student cannot read unrelated customer credit');
reset role;
select set_config('request.jwt.claim.sub','',true);
insert into auth.users(id) values('ee000000-0000-0000-0000-000000000004');
insert into profiles(id) values('ee000000-0000-0000-0000-000000000004');
insert into user_roles(user_id,role_id) select 'ee000000-0000-0000-0000-000000000004',id from roles where code='PARENT';
insert into parents(id,user_id,parent_code) values('ee900000-0000-0000-0000-000000000004','ee000000-0000-0000-0000-000000000004','CREDIT-PORTAL-P');
insert into student_parents(student_id,parent_id,can_view_finance) select id,'ee900000-0000-0000-0000-000000000004',false from fixture where name='portal-student';
set local role authenticated;
select set_config('request.jwt.claim.sub','ee000000-0000-0000-0000-000000000004',true);
select is((select count(*) from student_customer_credits((select id from fixture where name='portal-student'))),0::bigint,'Parent finance flag denies customer credit');
select is((select count(*) from student_opening_balances((select id from fixture where name='portal-student'))),0::bigint,'Parent finance flag denies opening balances');
reset role;
select set_config('request.jwt.claim.sub','',true);
update student_parents set can_view_finance=true where parent_id='ee900000-0000-0000-0000-000000000004';
set local role authenticated;
select set_config('request.jwt.claim.sub','ee000000-0000-0000-0000-000000000004',true);
select is((select remaining_credit from student_customer_credits((select id from fixture where name='portal-student')) where credit_id=(select id from fixture where name='credit')),0::numeric,'Permitted parent reads canonical credit');
select ok(not has_function_privilege('anon','public.student_customer_credits(uuid,integer)','EXECUTE'),'Anonymous credit portal denied');
select ok(not has_function_privilege('anon','public.student_opening_balances(uuid,integer)','EXECUTE'),'Anonymous opening portal denied');
select * from finish();
rollback;
