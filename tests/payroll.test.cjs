...
test('approval requires confirmation and version',async()=>{const h=harness();await redirected(h.load(base+'actions.ts').payrollAction,{action:'transition',period:id(1),status:'APPROVED',version:'2',note:'Checked'});assert.equal(h.calls.length,1);await redirected(h.load(base+'actions.ts').payrollAction,{action:'transition',period:id(1),status:'APPROVED',version:'2',note:'Checked',confirm:'yes'});assert.equal(h.calls.at(-1).args.p_version,2)})
