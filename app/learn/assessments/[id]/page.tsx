...
import { assessmentStates, type DeliveredQuestion } from '@/lib/learning/question-types'
...
export default async function AssessmentAttempt({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string}>}) {
  ...
  const result=await db.from('learning_assessment_attempts').select('id,state,score,question_snapshot,policy_snapshot').eq('id',id).maybeSingle()
  ...
  return <main className="mx-auto max-w-3xl space-y-5 p-4 sm:p-6"><Link prefetch={false} href="/learn" className="text-blue-700 underline">Về học trực tuyến</Link><h1 className="text-2xl font-semibold">Bài đánh giá</h1><p>Trạng thái: {assessmentStates[a.state]||a.state}</p><p>Ngưỡng đạt: {a.policy_snapshot.threshold}%</p>{a.score!==null&&<p>Điểm: {a.score}%</p>}{p.error&&<p role="alert">Không nộp được bài. Kiểm tra quyền học và thử lại.</p>}
    ...
    {a.state==='IN_PROGRESS'?<form action={submitAssessment} className="space-y-5"><input type="hidden" name="attempt" value={id}/>{questions.map(q=><fieldset key={q.code} className="space-y-3 rounded border p-4"><legend className="whitespace-pre-wrap break-words font-medium">{q.prompt} ({q.points} điểm)</legend>
      {q.mode==='AUTO'&&['SINGLE_CHOICE','MULTIPLE_CHOICE'].includes(q.type)?q.options?.map((option,index)=><label key={index} className="flex items-start gap-2"><input className="mt-1" type={q.type==='MULTIPLE_CHOICE'?'checkbox':'radio'} name={'answer.'+q.code} value={option}/><span className="break-words">{option}</span></label>):q.mode==='AUTO'&&q.type==='TRUE_FALSE'?<><label className="flex gap-2"><input type="radio" name={'answer.'+q.code} value="true"/>Đúng</label><label className="flex gap-2"><input type="radio" name={'answer.'+q.code} value="false"/>Sai</label></>:<label className="block">Câu trả lời (giáo viên sẽ chấm)<textarea className="block w-full rounded border p-2" name={'answer.'+q.code} maxLength={10000}/></label>}
