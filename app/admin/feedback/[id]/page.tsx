...
import { feedbackDetail, feedbackHistory, feedbackReasons, respondents, states } from '../data'
...
export default async function FeedbackDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Params>}) {
 ...
 let f,events
 let reasons: {reason_code:string;reason_label:string}[]=[]
 try {f=await feedbackDetail(db,id);events=f?await feedbackHistory(db,id):[];reasons=f?await feedbackReasons(db,id):[]} catch {return <LoadError/>}
 ...
 <Panel title="Đánh giá gốc"><p className={f.is_low_rating?'font-semibold text-red-800':''}>★ {f.overall_rating}/5{f.is_low_rating?' • Điểm thấp':''}</p><ul>{reasons.length?reasons.map(reason=><li key={reason.reason_code}>{reason.reason_label}</li>):<li>Không chọn lý do có sẵn</li>}</ul><p className="whitespace-pre-wrap">{f.comment||'Không có nhận xét'}</p><p>Nội dung gốc không đổi khi xử lý hoặc mở lại.</p></Panel>
