...
export default async function PayrollPeriod({params,searchParams}:{params:Promise<{period:string}>;searchParams:Promise<Params>}) {
 ...
 <Table headers={['Nhân viên / giáo viên','Loại','Giờ dạy','Lương tháng','Thu nhập buổi / giờ','Điều chỉnh','Tổng']} rows={list.data.map(t=>[<Link prefetch={false} key={t.id} href={'/admin/payroll/'+id+'/'+(t.teacher_id||t.employee_id)}>{t.teacher_name}</Link>,t.pay_type,t.teaching_hours,money(t.base_salary,t.currency),money(t.hourly_earnings,t.currency),money(t.adjustment_amount,t.currency),money(t.gross_amount,t.currency)])}/><Pager path={'/admin/payroll/'+id} params={p} {...list}/>
