...
export default async function PayrollPeriod({params,searchParams}:{params:Promise<{period:string}>;searchParams:Promise<Params>}) {
 ...
 try {loaded=await Promise.all([payrollList(db,id,p),teachers(db),rows(db.from('payroll_events').select('id,status,note,actor_id,created_at').eq('period_id',id).order('created_at',{ascending:false}).limit(50))])}catch{return <LoadError/>}
 ...
 {transitions.length>0&&<Panel title="Kiểm tra và chốt kỳ"><p>Quay về bản nháp để tính lại trước khi thêm điều chỉnh. Duyệt/chốt giữ nguyên số liệu; nếu người tạo tự duyệt, hệ thống ghi rõ trong lịch sử.</p><form action={payrollAction} className="space-y-3"><input type="hidden" name="action" value="transition"/><input type="hidden" name="period" value={id}/><input type="hidden" name="version" value={head.version}/><Select name="status" label="Chuyển trạng thái" options={states.filter(s=>transitions.includes(s.id))} required/><Field name="note" label="Ghi chú kiểm tra"/><Confirm text="Tôi đã kiểm tra số liệu và xác nhận chuyển trạng thái"/><SubmitButton>Lưu trạng thái kỳ lương</SubmitButton></form></Panel>}
 ...
 <Panel title="Lịch sử kỳ lương"><Table headers={['Trạng thái','Ghi chú','Người thực hiện']} rows={events.map(e=>[e.status,e.note,e.actor_id])}/></Panel></div>
}
