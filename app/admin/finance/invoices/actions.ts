...
export async function cancelInvoice(form: FormData) { if(form.get('invoice_status')!=='DRAFT') await submitFinancialRequest(form,'CANCEL_INVOICE',true); await mutate('invoices', 'cancel_invoice', form, { invoice_id: 'id', reason: 'required' }, { confirm: true }) }
