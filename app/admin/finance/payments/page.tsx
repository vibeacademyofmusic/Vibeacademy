...
import { paymentList, paymentDetail, allocationInvoices, total } from './data'
import { createPayment, allocatePayment, voidPayment } from './actions'
...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  try {
    ...
    const [invoices, names] = await Promise.all([detail ? allocationInvoices(db, detail.payment, params) : null, studentNames(db, [...list.data.map(p => p.student_id_snapshot), ...(detail ? [detail.payment.student_id_snapshot] : [])])])
    loaded = { list, branchRows, students, detail, invoices, names }
  } catch { return <LoadError /> }
  const { list, branchRows, students, detail, invoices, names } = loaded
  ...
  return <div className="min-w-0 space-y-6"><h1 className="text-3xl font-bold">Thanh toán</h1><Notice params={params} />
    ...
    <Table headers={['Thanh toán', 'Học viên', 'Chi nhánh', 'Tiền tệ', 'Số tiền', 'Phương thức', 'Ngày thu', 'Tham chiếu', 'Trạng thái', 'Phân bổ gốc', 'Chưa phân bổ']} rows={list.data.map(p => {
      const allocated = total(list.allocations.filter(a => a.payment_id === p.id))
      return [<Link prefetch={false} key={p.id} className="underline" href={'?selected=' + p.id}>{p.payment_number}</Link>, names.get(p.student_id_snapshot), p.branch_name_snapshot, p.currency, money(p.amount, p.currency), p.payment_method, timeText(p.paid_at), p.reference ?? '—', p.status, money(allocated, p.currency), p.status === 'POSTED' ? money(Number(p.amount) - allocated, p.currency) : '—']
    ...
    {payment && detail && <Panel title={'Thanh toán ' + payment.payment_number}><p>{names.get(payment.student_id_snapshot)} • {money(payment.amount, payment.currency)} • {payment.status}</p>
      <p>Đã phân bổ: {money(total(detail.allocations), payment.currency)} • Đã hoàn: {money(total(detail.refunds), payment.currency)}</p>
