...
export default async function RefundsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div className="min-w-0 space-y-6"><h1 className="text-3xl font-bold">Hoàn tiền</h1><Notice params={params} />
    ...
    {detail && <Panel title={'Thanh toán gốc ' + detail.payment.payment_number}>
      ...
      {!refund && detail.payment.status === 'POSTED' && remaining > 0 && <form action={createRefund} className="grid gap-3 sm:grid-cols-2"><input type="hidden" name="payment_id" value={detail.payment.id} /><Field name="amount" label="Số tiền hoàn" type="number" max={remaining} /><Field name="refunded_at" label="Thời gian hoàn (giờ Việt Nam)" type="datetime-local" value={vietnamDateTime()} /><Field name="reason" label="Lý do hoàn tiền" /><Field name="notes" label="Ghi chú" required={false} /><Confirm text="Xác nhận ghi nhận tiền đã hoàn. Sau khi tạo, chọn phân bổ gốc để cập nhật công nợ của hóa đơn tương ứng." /><SubmitButton>Tạo phiếu hoàn tiền</SubmitButton></form>}
      ...
      <Table headers={['Hóa đơn gốc', 'Phân bổ gốc', 'Đã hoàn vào phân bổ', 'Thao tác']} rows={originalRows.map(a => {
        ...
        return [a.invoices.invoice_number, money(a.amount, detail.payment.currency), money(refunded, detail.payment.currency), refund?.status === 'POSTED' && available > 0 && !allocations.some(r => r.refund_id === refund.id && r.payment_allocation_id === a.id) && Number(refund.amount) > total(allocations.filter(r => r.refund_id === refund.id)) ? <form key={a.id} action={allocateRefund} className="space-y-2"><input type="hidden" name="refund_id" value={refund.id} /><input type="hidden" name="selected" value={refund.id} /><input type="hidden" name="payment_allocation_id" value={a.id} /><Field name="amount" label={'Hoàn vào ' + a.invoices.invoice_number} type="number" max={Math.min(available, Number(refund.amount) - total(allocations.filter(r => r.refund_id === refund.id)))} /><SubmitButton>Phân bổ hoàn tiền</SubmitButton></form> : '—']
      })} />
      ...
      {refund?.status === 'POSTED' && <form action={voidRefund} className="space-y-3"><input type="hidden" name="refund_id" value={refund.id} /><input type="hidden" name="selected" value={refund.id} /><Field name="reason" label="Lý do vô hiệu hoàn tiền" /><Confirm text="Vô hiệu phiếu hoàn sẽ hoàn tác ảnh hưởng của phiếu lên công nợ. Tôi xác nhận thao tác." /><SubmitButton>Vô hiệu hoàn tiền</SubmitButton></form>}
    </Panel>}
  </div>
}
