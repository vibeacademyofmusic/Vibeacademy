...
import { branches, studentNames, rows } from '../query'
import { pageNumber, pageSize, type Params, vietnamDateTime } from '../operations'
import { paymentList, paymentDetail, total, effectiveAllocationTotal } from '../payments/data'
...
export default async function RefundsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  try {
    ...
    const creditRefund = refund ? (await rows(db.from('customer_credit_uses').select('id').eq('refund_id', refund.id).limit(1))).length > 0 : false
    ...
    loaded = { list, branchRows, payments, refund, detail, allocations, names, creditRefund }
  } catch { return <LoadError /> }
  const { list, branchRows, payments, refund, detail, allocations, names, creditRefund } = loaded
  ...
  return <div className="min-w-0 space-y-6"><h1 className="text-3xl font-bold">Hoàn tiền</h1><Notice params={params} />
    ...
    {detail && <Panel title={'Thanh toán gốc ' + detail.payment.payment_number}>
      ...
      <p>Số tiền: {money(detail.payment.amount, detail.payment.currency)} • Phân bổ gốc: {money(effectiveAllocationTotal(detail.allocations), detail.payment.currency)} • Đã hoàn: {money(total(detail.refunds), detail.payment.currency)} • Còn có thể hoàn: {money(remaining, detail.payment.currency)}</p>
      ...
      {refund && <><Link prefetch={false} className="underline" href={'/documents/finance/refunds/' + refund.id}>Xem / In chứng từ</Link><h3 className="font-semibold">{refund.refund_number} • {refund.status} • {money(refund.amount, refund.currency)}</h3>{creditRefund ? <p>Phiếu hoàn số dư khách hàng đã gắn với credit gốc. Không phân bổ lại vào công nợ; vô hiệu phiếu sẽ khôi phục credit.</p> : <p>Phần hoàn chưa phân bổ: {refund.status === 'POSTED' ? money(Number(refund.amount) - total(allocations.filter(a => a.refund_id === refund.id)), refund.currency) : 'Phiếu đã vô hiệu'}</p>}</>}
      <h3 className="font-semibold">Phân bổ gốc nhận hoàn</h3>
      <Table headers={['Hóa đơn / mở sổ', 'Phân bổ gốc', 'Đã hoàn vào phân bổ', 'Thao tác']} rows={originalRows.map(a => {
        const refunded = total(allocations.filter(r => r.payment_allocation_id === a.id))
        const available = Number(a.amount) - Number(a.released_to_credit ?? 0) - refunded
        return [(a.invoices?.invoice_number ?? 'Công nợ mở sổ'), money(Number(a.amount) - Number(a.released_to_credit ?? 0), detail.payment.currency), money(refunded, detail.payment.currency), !creditRefund && refund?.status === 'POSTED' && available > 0 && !allocations.some(r => r.refund_id === refund.id && r.payment_allocation_id === a.id) && Number(refund.amount) > total(allocations.filter(r => r.refund_id === refund.id)) ? <form key={a.id} action={allocateRefund} className="space-y-2"><input type="hidden" name="idempotency_key" value={randomUUID()}/><input type="hidden" name="refund_id" value={refund.id} /><input type="hidden" name="selected" value={refund.id} /><input type="hidden" name="payment_allocation_id" value={a.id} /><Field name="amount" label={'Hoàn vào ' + (a.invoices?.invoice_number ?? 'Công nợ mở sổ')} type="number" max={Math.min(available, Number(refund.amount) - total(allocations.filter(r => r.refund_id === refund.id)))} /><Field name="reason" label="Lý do phân bổ hoàn tiền"/><Confirm text="Gửi phân bổ để người khác phê duyệt"/><SubmitButton>Yêu cầu phân bổ hoàn tiền</SubmitButton></form> : '—']
      })} />
