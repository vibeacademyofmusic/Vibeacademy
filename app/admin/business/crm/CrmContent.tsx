...
export async function CrmLeadContent({ searchParams }: { searchParams: Promise<LeadFilters & { error?: string; success?: string }> }) {
  ...
  return (
    <div>
      ...
      {filters.error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">{filters.error}</div>}
      {list.error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">Không tải được khách hàng: {list.error}</div>}
