...
import { sourceLabel, statusLabel, tabs } from './model'
...
function hrefFor(filters: LeadFilters, patch: LeadFilters) {
  ...
  return query ? `/admin/business/crm?${query}` : '/admin/business/crm'
}

export default async function CrmLeadPage({ searchParams }: { searchParams: Promise<LeadFilters & { error?: string; success?: string }> }) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className="mt-1 text-3xl font-bold tracking-tight text-gray-950">Khách hàng mới</h1>
      </div>
      ...
      <section className="mb-6">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {directory.queue.map(item => (
            <Link key={item.id} href={hrefFor(filters, { queue: item.id, tab: undefined })} className="rounded-2xl border border-gray-200 bg-white px-4 py-3">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {tabs.map(tab => (
          <Link key={tab.id} href={hrefFor(filters, { tab: tab.id === 'all' ? undefined : tab.id, queue: undefined })} className={`rounded-full px-3 py-1.5 text-sm ${((filters.tab || 'all') === tab.id) ? 'bg-gray-950 text-white' : 'bg-white text-gray-700 ring-1 ring-gray-200'}`}>{tab.label}</Link>
        ))}
      </div>
      <form className="mb-4 grid gap-3 rounded-2xl border border-gray-200 bg-white p-4 md:grid-cols-3 xl:grid-cols-6">
        ...
        <section className="overflow-x-auto rounded-2xl border border-gray-200 bg-white">
          <table className="w-full min-w-[880px] text-left text-sm">
            <thead className="border-b border-gray-200 text-gray-500">
              <tr>
                {['Tên', 'Điện thoại', 'Email', 'Nguồn', 'Bộ môn / nhạc cụ', 'Chi nhánh', 'Owner', 'Trạng thái', 'Lần liên hệ cuối', 'Follow-up tiếp theo'].map(label => <th key={label} className="px-3 py-3 font-medium">{label}</th>)}
              </tr>
            </thead>
            <tbody>
              {list.rows.length === 0 && <tr><td colSpan={10} className="px-3 py-8 text-gray-500">Chưa có khách hàng trong bộ lọc này.</td></tr>}
              {list.rows.map(row => (
