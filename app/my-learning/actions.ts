...
import { validFeedbackReasons } from '@/app/feedback/reasons'
...
export async function submitFeedback(form: FormData) {
  ...
  const reasons = form.getAll('reason').map(value => String(value))
  ...
  if (!['STUDENT', 'PARENT'].includes(respondent) || !/^[1-5]$/.test(rating) || comment.length > 4000 || !validFeedbackReasons(Number(rating), reasons)) redirect(path + '&error=feedback_invalid')
  const result = await db.rpc('submit_lesson_feedback', { p_student_id: student, p_session_id: session, p_respondent_type: respondent, p_overall: Number(rating), p_comment: comment, p_reasons: reasons })
