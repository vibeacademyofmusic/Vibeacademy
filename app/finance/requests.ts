...
export function financialRequestInput(form: FormData, operation: string) {
  ...
  const targetField = operation === 'CANCEL_INVOICE' ? 'invoice_id' : operation === 'PAYROLL_CORRECTION' ? 'payroll_id' : ['VOID_REFUND', 'ALLOCATE_REFUND'].includes(operation) ? 'refund_id' : 'payment_id'
...
export function refreshFinance() {
  for (const path of ['/finance', '/admin/finance', '/admin/payroll']) revalidatePath(path, 'layout')
}
