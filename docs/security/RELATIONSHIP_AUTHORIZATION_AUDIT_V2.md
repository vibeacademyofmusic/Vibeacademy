...
Finance/Payroll read increment `20260916095000`: FINANCE SELECT requires its own finance.view/payroll.view permission and matching active assignment scope. Invoice/payment/refund parents and allocation rows are branch-filtered; receivables reuse the existing security-invoker engine. Payroll periods/rows/compensation/events allow scoped FINANCE reads; teacher approved/finalized self-read additionally requires payroll.view_own. No user assignments, direct DML privileges or write RPCs were changed. Local **39 files / 735 pgTAP**, build, 103 app/bootstrap tests and relevant ESLint PASS. Staging replay **49 Finance + 31 Payroll assertions PASS**. Full FINANCE navigation, tuition/forecast projections and academic staff report delegation are not certified by these read-policy tests.

Open owner decisions before delegated financial approvals: refund/payroll maker-checker thresholds and SUPER_ADMIN emergency override. Security V1 explicitly leaves these for owner confirmation; the existing mutation workflows have not been expanded under an assumed threshold. Production remains HOLD.
